...
func send_broadcast(message) -> int:
	var type = typeof(message)
	for id in peers:
		# FIXME: return error
		assert(peers.has(id), "Peer ID not found")
		...
		if type == TYPE_STRING:
			...
			# NOTE: You can't send var_to_bytes()'s bytes over the network...
			# return socket.send(var_to_bytes(message))
			return ERR_INVALID_DATA
			
	# It is okay if there are no peers to broadcast to...
	return OK
